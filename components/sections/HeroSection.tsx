...
const leftColumnCards = [
  ...
  { title: "Product Manager", flag: "🇨🇱", image: "/figmaAssets/product-manager.png" },
  { title: "Data Analyst", flag: "🇺🇾", image: "/figmaAssets/data-analyst.png" },
...
type TalentCardProps = { title: string; flag: string; image: string };

const TalentCard = ({ title, flag, image }: TalentCardProps) => {
  return (
        ...
        <header className="flex w-full max-w-[722px] flex-col gap-8 lg:pt-[61px]">
          <div className="flex flex-col items-start gap-[21.3px]">
            ...
            <div className="flex flex-wrap items-start gap-4">
              ...
              <Button variant="outline" className="h-auto rounded-[999px] border-[#f5efe629] bg-transparent px-[22px] py-3.5 [font-family:'Inter',Helvetica] text-[14.5px] font-medium text-[#f5efe6] hover:bg-white/5 hover:text-[#f5efe6]">
                Free Consultation
              </Button>
            </div>
          </div>
          ...
          <div className="absolute left-0 top-[10px] flex h-[800px] w-[200px] overflow-hidden">
            <div className="flex w-[200px] flex-col items-start gap-[10.92px]">
              {leftColumnCards.map((card, index) => (
                <TalentCard key={`${card.title}-${card.flag}-${index}`} title={card.title} flag={card.flag} image={card.image} />
              ))}
              <Image width={200} height={200} alt="Background shadow" src="/figmaAssets/background-shadow-1.svg" className="w-full" />
            </div>
          </div>
          <div className="absolute right-0 top-[10px] flex h-[800px] w-[200px] items-end overflow-hidden">
            <div className="flex w-[200px] flex-col items-start gap-[10.92px]">
              <Image width={200} height={200} alt="Background shadow" src="/figmaAssets/background-shadow.svg" className="w-full" />
