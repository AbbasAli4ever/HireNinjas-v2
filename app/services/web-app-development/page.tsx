...
const principles = [
  { icon: "⚡", title: "Built for Performance", description: "Every product we ship is optimised for speed, SEO and Core Web Vitals — because slow sites cost you customers." },
  { icon: "📱", title: "Mobile-First Always", description: "We design and build with mobile as the primary experience, ensuring your product works flawlessly on every screen size." },
  { icon: "🔒", title: "Scalable & Secure", description: "Our architecture decisions anticipate growth — built to handle increasing load without rebuilding from scratch." },
];

const processSteps = [
  { number: "01", title: "Discovery & Scoping", description: "We audit your requirements, define user journeys and map the full technical scope before writing a single line of code." },
  { number: "02", title: "UI/UX Design", description: "Wireframes, prototypes and full design systems built in Figma — tested with real users before development begins." },
  { number: "03", title: "Development Sprints", description: "Agile two-week sprints with regular demos so you see progress early and can provide feedback throughout." },
  { number: "04", title: "QA & Testing", description: "Cross-device, cross-browser testing with performance audits and security reviews before every launch." },
  { number: "05", title: "Launch & Support", description: "Smooth deployment with full handover documentation, training and ongoing support packages available." },
];
        ...
        <section className="relative w-full px-4 py-[80px] sm:px-6 lg:px-[140px]">
          <div className="mx-auto w-full max-w-[1200px]">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {services.map((s) => (
                <article key={s.title} className="flex flex-col gap-3 rounded-[16px] border border-[#ece8f3] bg-white p-6 transition-shadow hover:shadow-[0px_4px_20px_rgba(139,58,229,0.1)]">
                  <span className="inline-flex w-fit items-center rounded-[999px] bg-[#f8f1ff] px-3 py-1 [font-family:'Montserrat',Helvetica] text-[11px] font-semibold tracking-[0.5px] text-[#591f97]">{s.tag}</span>
                ...
                <div key={stack.category} className="flex flex-col gap-3 rounded-[16px] border border-[#ece8f3] bg-[#fafaff] p-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {stack.items.map((item) => (
                      <span key={item} className="rounded-[8px] border border-[#ece8f3] bg-white px-3 py-1.5 [font-family:'Montserrat',Helvetica] text-[13px] font-medium text-[#0f0a1f]">{item}</span>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <div className="mx-auto w-full max-w-[1200px]">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {deliverables.filter((d) => !d.full).map((item) => (
                <article key={item.title} className={`flex flex-col gap-3 rounded-[16px] border border-[#ece8f3] bg-white p-6 ${item.wide ? "sm:col-span-2 lg:col-span-1" : ""}`}>
