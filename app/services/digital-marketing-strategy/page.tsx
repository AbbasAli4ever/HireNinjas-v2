...
const strategyPillars = [
  { number: "01", title: "Brand Positioning & Acquisition", description: "Sharpen who you are and how you win attention in the UAE market." },
  { number: "02", title: "Conversion Optimization", description: "Tighten funnels so every dollar drives more pipeline and revenue." },
  { number: "03", title: "Reporting Framework", description: "Decision-grade dashboards built on the metrics that matter to growth." },
  { number: "04", title: "Long-term Scalability", description: "Systems that compound over time — not just quick wins that fade." },
];

const principles = [
  { icon: "📊", title: "Data-Led Decisions", description: "Every recommendation we make is grounded in real audience data, competitor intelligence and channel analytics — not assumptions." },
  { icon: "🇦🇪", title: "UAE Market Expertise", description: "We understand Dubai and Abu Dhabi buyer behaviour, platform nuances and local market dynamics that generic agencies simply don't." },
  { icon: "📈", title: "Full-Funnel Accountability", description: "We tie every activity to revenue outcomes. Strategy without measurable results is just a document. We build for compounding growth." },
];

const roadmapItems = [
  { title: "Competitor Intelligence Report", description: "Deep analysis of your top competitors' positioning, messaging, channels and spend to uncover gaps you can exploit.", tag: "Research", wide: true },
  { title: "Audience Behaviour Map", description: "Detailed segmentation of your ideal customers — their triggers, objections, platforms and purchase journey stages.", tag: "Insights", wide: true },
  { title: "Channel Strategy Blueprint", description: "A precise breakdown of which channels to activate, in what order, with what budget allocation.", tag: "Strategy", wide: false },
  { title: "90-Day Growth Roadmap", description: "A phased, prioritised action plan with clear milestones and expected outcomes at each stage.", tag: "Planning", wide: false },
  { title: "KPI & Reporting Framework", description: "Custom dashboards and tracking infrastructure so you always know what's working.", tag: "Analytics", wide: false },
  { title: "Quarterly Strategy Reviews", description: "Regular refinement sessions with our senior strategists to adapt your roadmap as the market evolves.", tag: "Ongoing", wide: true, full: true },
];

const processSteps = [
  { number: "01", title: "Discovery & Audit", description: "We review your current marketing, analytics, competitors and market position to establish a clear baseline." },
  { number: "02", title: "Strategy Development", description: "Our strategists build a custom growth blueprint aligned to your commercial goals and UAE market context." },
  { number: "03", title: "Channel Planning", description: "We map the right mix of channels, content formats and budget allocation to maximise ROI." },
  { number: "04", title: "Launch & Execute", description: "Campaigns go live with full tracking in place — every touchpoint measured from day one." },
  { number: "05", title: "Optimise & Scale", description: "Continuous refinement based on live data drives compounding performance over time." },
];
    ...
    <div className="w-full bg-white">
      ...
      <main>
        ...
        <section className="relative w-full overflow-hidden bg-white pb-0 pt-[96px]">
          ...
          <div className="relative z-10 mx-auto flex w-full max-w-[1160px] flex-col items-start gap-8 px-4 sm:px-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex w-full max-w-[640px] flex-col gap-6 lg:pb-[80px]">
              ...
              <div className="flex flex-wrap items-center gap-4">
                <Button className="h-auto rounded-[999px] bg-[#8b39e5] px-2 py-2 pl-[22px] pr-2 hover:bg-[#7f32d5]">
                ...
                <Button variant="outline" className="h-auto rounded-[999px] border-[#0f0a1f29] bg-transparent px-[22px] py-3.5 [font-family:'Inter',Helvetica] text-[14.5px] font-medium text-[#0f0a1f] hover:bg-[#f5f0ff]">
                  Free Consultation
                </Button>
              </div>
              <div className="flex flex-wrap gap-6 border-t border-[#ece8f3] pt-6 sm:gap-10">
                {[
                  ...
                  <div key={stat.label} className="flex flex-col border-l-2 border-[#8b3ae5] pl-4">
                    <span className="[font-family:'Satoshi-Black',Helvetica] text-[22px] font-black leading-tight tracking-[-0.5px] text-[#0f0a1f]">{stat.value}</span>
                    <span className="[font-family:'Montserrat',Helvetica] text-[13px] font-normal leading-[19px] text-[#6b6478]">{stat.label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mx-auto w-full max-w-[1160px] px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-16">
              <div className="flex flex-col gap-5">
                ...
                <p className="[font-family:'Montserrat',Helvetica] text-base font-normal leading-[25.6px] text-[#3a3447]">
                  Our strategy process leverages competitor intelligence, audience behavior insights, keyword opportunity mapping, and channel performance forecasting to create a strategic growth blueprint tailored specifically to your commercial goals.
                </p>
                <p className="[font-family:'Montserrat',Helvetica] text-base font-normal leading-[25.6px] text-[#3a3447]">
          ...
          <div className="mx-auto w-full max-w-[1160px] px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {roadmapItems.filter((i) => !i.full).map((item) => (
                <article key={item.title} className={`flex flex-col gap-3 rounded-[16px] border border-[#ece8f3] bg-white p-6 shadow-[0px_2px_8px_rgba(139,58,229,0.05)] ${item.wide ? "sm:col-span-2 lg:col-span-1" : ""}`}>
            ...
            {roadmapItems.filter((i) => i.full).map((item) => (
              <article key={item.title} className="mt-4 flex flex-col gap-3 rounded-[16px] border border-[#ece8f3] bg-[#fafaff] p-6 sm:flex-row sm:items-center sm:justify-between sm:gap-8">
          ...
          <div className="mx-auto w-full max-w-[1160px] px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
              {processSteps.map((step) => (
                <article key={step.number} className="flex flex-col gap-4 rounded-[16px] border border-[#ece8f3] bg-white p-6">
